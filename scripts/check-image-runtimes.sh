...
docker run --rm --entrypoint /bin/sh "$image" -c 'go tool compile -V'
docker run --rm --entrypoint /bin/sh "$image" -c 'crane version'
docker run --rm --entrypoint /bin/sh "$image" -c 'docker --version'
docker run --rm --entrypoint /bin/sh "$image" -c 'test "$GOROOT" = /usr/local/go'
...
docker run --rm --entrypoint /bin/sh "$image" -c 'test -f /usr/local/share/smoothnas-go-toolchain/go.tar.gz.part.000'
docker run --rm --entrypoint /bin/sh "$image" -c 'test -f /usr/local/share/smoothnas-toolchain/crane/crane.part.000'
docker run --rm --entrypoint /bin/sh "$image" -c 'test -f /usr/local/share/smoothnas-toolchain/docker/docker.part.000'
