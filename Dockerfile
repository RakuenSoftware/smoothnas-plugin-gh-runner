...
# Bake the upstream llama.cpp source used by the llama-cpp plugin release
# workflow. Worker jobs build from this local tree instead of cloning or
# downloading source at job runtime.
RUN set -eux; \
    resolved_ref="${LLAMA_REF}"; \
    if [ "$resolved_ref" = latest ]; then \
      resolved_ref="$(curl -fsSL https://api.github.com/repos/ggml-org/llama.cpp/releases/latest | jq -r .tag_name)"; \
    fi; \
    test -n "$resolved_ref"; \
    curl -fsSLo /tmp/llama.tar.gz \
      "${LLAMA_REPO}/archive/${resolved_ref}.tar.gz"; \
    if [ -n "${LLAMA_ARCHIVE_SHA256}" ]; then \
      echo "${LLAMA_ARCHIVE_SHA256}  /tmp/llama.tar.gz" | sha256sum -c -; \
    fi; \
    mkdir -p /opt/llama.cpp; \
    tar -xzf /tmp/llama.tar.gz --strip-components=1 -C /opt/llama.cpp; \
    printf '%s\n' "$resolved_ref" > /opt/llama.cpp/.smoothnas-llama-ref; \
    printf '%s\n' "${LLAMA_REPO}" > /opt/llama.cpp/.smoothnas-llama-repo; \
    rm /tmp/llama.tar.gz; \
...
RUN set -eux; \
    ...
    go_backup_dir=/usr/local/share/smoothnas-go-toolchain; \
    ...
    cp "$crane_path" /tmp/smoothnas-crane; \
    install -m 0755 /tmp/smoothnas-crane /usr/local/bin/crane; \
    rm -f /tmp/smoothnas-crane; \
    cp "$docker_path" /tmp/smoothnas-docker; \
    install -m 0755 /tmp/smoothnas-docker /usr/local/bin/docker; \
    rm -f /tmp/smoothnas-docker; \
    cp "$nvcc_path" /tmp/smoothnas-nvcc; \
    install -m 0755 /tmp/smoothnas-nvcc /usr/local/cuda/bin/nvcc; \
    rm -f /tmp/smoothnas-nvcc; \
    cp "$ptxas_path" /tmp/smoothnas-ptxas; \
    install -m 0755 /tmp/smoothnas-ptxas /usr/local/cuda/bin/ptxas; \
    rm -f /tmp/smoothnas-ptxas; \
    cp "$nvlink_path" /tmp/smoothnas-nvlink; \
    install -m 0755 /tmp/smoothnas-nvlink /usr/local/cuda/bin/nvlink; \
    rm -f /tmp/smoothnas-nvlink; \
    install -m 0755 "$cc1_path" /usr/local/bin/cc1; \
    install -m 0755 "$cc1plus_path" /usr/local/bin/cc1plus; \
    install -m 0755 "$cicc_path" /usr/local/bin/cicc; \
    mkdir -p /usr/local/share/cuda-nvvm/libdevice; \
    ...
    backup_toolchain_file crane /usr/local/bin/crane 0755; \
    backup_toolchain_file docker /usr/local/bin/docker 0755; \
    ...
    mkdir -p "$go_backup_dir"; \
    tar -C /usr/local -czf "$go_backup_dir/go.tar.gz" go; \
    split -b 8m -d -a 3 "$go_backup_dir/go.tar.gz" "$go_backup_dir/go.tar.gz.part."; \
    chmod 0644 "$go_backup_dir/go.tar.gz.part."*; \
    test -x /usr/local/go/bin/go; \
    test -x /usr/local/go/pkg/tool/linux_amd64/compile; \
    ...
    test -f /usr/local/share/smoothnas-go-toolchain/go.tar.gz.part.000; \
    test -f /usr/local/share/smoothnas-toolchain/go/go.part.000; \
    test -f /usr/local/share/smoothnas-toolchain/crane/crane.part.000; \
    test -f /usr/local/share/smoothnas-toolchain/docker/docker.part.000; \
